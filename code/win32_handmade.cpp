...
static void win32_getFileName(win32_State *state) {
    DWORD sizeOfFileName = GetModuleFileNameA(0, state->fileName,sizeof(state->fileName));
...
static void win32_getInputFileLocation (win32_State *state ,bool32 isInput, int slotIndex,int destCount,char *dest) 
{
    char temp[64];
...
static win32_ReplayBuffer *win32_getReplayBuffer(win32_State *state, int unsigned index){
...
{ 
    ...
    if (replayBuffer->memoryBlock) {
        ...
        //DWORD bytesRead;
        //ReadFile(state->playbackHandle, state->gameMemoryBlock, (DWORD)state->totalSize,&bytesRead,0);
        //
        char fileName[WIN32_STATE_FILE_NAME_COUNT];
        ...
        GameControllerInput *keyboardController)
{
    ...
    {
        float result =((end.QuadPart - start.QuadPart)  / (float) g_perfCountFrequency); 
        return result;
    }
            ...
            )
    {
        ...
        win32LoadInput();
        win32_GameCode gameCode = win32LoadGameCode(sourceDllFullPath,tempDllFullPath); 
        ...
        bool32 schedulerConfigured  = (timeBeginPeriod(desiredSchedulerMS) == TIMERR_NOERROR);
        ...
        if (RegisterClass(&WindowClass))    {
            ...
            if(hwnd) {
                ...
                monitorRefreshHz = (monitorRefreshHz > 1) ? monitorRefreshHz : 60;
                ...
                soundOutput.safetyBytes = (int)((float)(soundOutput.bytesPerSample*soundOutput.samplesPerSecond)/ gameUpdateHz/3.0f);
                ...
                gameMemory.transientStorage = (uint8_t *)state.gameMemoryBlock + gameMemory.permanentStorageSize ;
                gameMemory.isInitialized = false;

                for (int replayIndex =0; 
                        replayIndex < ARRAY_COUNT(state.replayBuffers);
                        replayIndex++){
                    win32_ReplayBuffer *replayBuffer = &state.replayBuffers[replayIndex];

                    win32_getInputFileLocation(&state, false,replayIndex,sizeof(replayBuffer->fileName),replayBuffer->fileName);
                    ...
                    //replayBuffer->memoryBlock = VirtualAlloc(0,(size_t)state.totalSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
                    if (replayBuffer->memoryBlock){
                ...
                if(samples && gameMemory.permanentStorage && gameMemory.transientStorage){
                    ...
                    for (;g_running;){
                        ...
                        FILETIME newDllWriteTime = win32_getFileDate(sourceDllFullPath);
                        ...
                        *newKeyboardController = {};
                        ...
                        if (!g_pause){
                            ...
                            if (gameCode.updateAndRender) {
                                gameCode.updateAndRender(&context,&gameMemory, &frameBuffer,newInput);
                            }
                            ...
                            if (g_secondaryBuffer->GetCurrentPosition(&playCursor,&writeCursor) == DS_OK) {
                                ...
                                DWORD expectedFrameBoundary = playCursor + expectedBytesUntilFlip;
                                ...
                                bool32 audioCardIsLowLatency= (safetyWriteCursor < expectedFrameBoundary); 
                                ...
                                if (gameCode.getSoundSamples) {
                                    gameCode.getSoundSamples(&context,&gameMemory,&soundBuffer);
                                }
                            ...
                            float secondElapsedPerFrame = win32getSecondsElapsed(lastCounter,win32getWallClock()); 

                            if (secondElapsedPerFrame < targetSecondsPerFrame) {
                                if (schedulerConfigured){
                                    DWORD ms = (DWORD) (1000.0f*(targetSecondsPerFrame 
                                ...
                                while (secondElapsedPerFrame < targetSecondsPerFrame) {
                                    secondElapsedPerFrame = win32getSecondsElapsed(lastCounter,
                                            win32getWallClock()); 
                                }
                            } else {
                                // Missed frame rate!!! 
                            ...
                            double FPS  = (double)g_perfCountFrequency /(double)win32getSecondsElapsed(lastCounter,win32getWallClock()); 
                            char buffer[256];
                            sprintf_s(buffer,"ms/loop %0.2f FPS %0.2f MegaCycles %0.2f\n",msPerFrame,FPS,MCPF);
